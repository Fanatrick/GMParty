...
void main() {
	...
	//vec4 plife = lookup(indexUVs, 0.0, gm_BaseTexture); // we don't need this here, as we're modifying life slot
	vec3 ppos = lookup(indexUVs, 1.0, gm_BaseTexture).xyz;
	
	if (pack.w != clamp(pack.w, ugmpParticleTypeRange.x, ugmpParticleTypeRange.y)) {
		discard;
	}
	...
	if (len > 0.0) {
		...
		pack.x = max(pack.x - len * max(acc, 0.0), 0.0);
	}
